...
def train(epoch):
    ...
    for batch in tqdm(training_data_loader):
        ...
        loss_hvi = CCL_loss(output_hvi, gt_hvi) + D_loss(output_hvi, gt_hvi) + E_loss(output_hvi, gt_hvi) + opt.P_weight * P_loss(output_hvi, gt_hvi)[0]
        loss_rgb = L2_loss_rgb(output_rgb, gt_rgb) + D_loss(output_rgb, gt_rgb) + E_loss(output_rgb, gt_rgb) + opt.P_weight * P_loss(output_rgb, gt_rgb)[0]
...
def checkpoint(epoch):
    if not os.path.exists("./weights"):          
        os.mkdir("./weights") 
    if not os.path.exists("./weights/train_lolv1"):          
        os.mkdir("./weights/train_lolv1")  
    model_out_path = "./weights/train_lolv1/epoch_{}.pth".format(epoch)
...
def build_model():
    ...
    if opt.start_epoch > 0:
        pth = f"./weights/train_lolv1/epoch_{opt.start_epoch}.pth"
...
def init_loss():
    CCL_weight = opt.CCl_weight
    L2_weight   = opt.L2_weight
    D_weight    = opt.D_weight 
    E_weight    = opt.E_weight 
    P_weight    = 1.0
    
    CCL_loss = CCL(loss_weight=CCL_weight, reduction='mean').cuda()
    L2_loss_rgb= L2Loss_rgb(loss_weight=L2_weight, reduction='mean').cuda()
    ...
    return CCL_loss,L2_loss_rgb,P_loss,E_loss,D_loss
...
if __name__ == '__main__':  
    ...
    CCL_loss,L2_loss_rgb,P_loss,E_loss,D_loss = init_loss()
